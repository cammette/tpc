...
public class TaskMonitor {
	...
	public void init() {
		...
		new Thread(browserQueueMonitorThread, "mbrowserQueueMonitorThread")
				.start();
				...
				try {
					...
					if (taskResults.size() > 0) {
						...
						for (TaskResult taskResult : taskResults) {
							...
							replyClientService.send(taskResult,
									taskResult.getBatchNo(), null);
						}
					}
		...
		public void run() {
			while (isRunning) {
				try {
					...
					for (TaskResult taskResult : taskResults) {
						// taskResult.setEndTime(new Date());
						// notifyClientService.send(taskResult,
						// taskResult.getBatchNo(),null);
						taskResult.setEndTime(new Date());
						replyClientService.send(taskResult,
								taskResult.getBatchNo(), null);
	...
	public void setSendAndReplyClientService(
			JMSService sendAndReplyClientService) {
