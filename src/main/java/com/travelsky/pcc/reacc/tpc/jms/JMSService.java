...
public class JMSService {
  ...
  public Message waitforReply(String batchNo, long timeout) throws TaskExcutedReplyTimeoutException {
    ...
    try {
      jmsConnectionSession = (JmsConnectionSession) replyJmsConnectionPool.getJmsConnectionSession();
      ...
      batchSelector.append("'");
      consumer = session.createConsumer(replyQueue, batchSelector.toString());
      Message replyMessage = consumer.receive(timeout);
      if (replyMessage == null) {
        throw new TaskExcutedReplyTimeoutException("waiting for the reply message timeout");
      }
      return replyMessage;
      ...
      if (jmsConnectionSession != null) {
        try {
          replyJmsConnectionPool.returnJmsConnectionSession(jmsConnectionSession);
        } catch (Exception e) {
          log.error("put the jmsConnectionSession to back the pool unsuccessfully", e);
        }
      }
    }
  }
  ...
  public void send(Serializable msg, String batchNo, Map<String, String> messageProperties, Integer priority) {
    ...
    try {
      jmsConnectionSession = (JmsConnectionSession) jmsConnectionPool.getJmsConnectionSession();
      Session session = jmsConnectionSession.getSession();
      ...
      if (jmsConnectionSession != null) {
        try {
          jmsConnectionPool.returnJmsConnectionSession(jmsConnectionSession);
        } catch (Exception e) {
          log.error("put the jmsConnectionSession to back the pool unsuccessfully", e);
        }
      }
